...
**Easy yt-dlp** 是一个专为全能视频下载利器 yt-dlp 设计的极简交互式前端工具。它完美解决了原版命令行参数繁多、记忆成本高的痛点，通过全自动化流程为您带来爽快的下载体验。自动安装、自动更新、独创格式偏好系统（Profile）能记住您的下载习惯；采用菜单驱动模式，只需粘贴 URL 即可轻松下载，对不熟悉终端的用户极度友好。一键安装（同时支持免安装运行），支持Windows、Linux和MacOS，安装后自动创建桌面快捷方式。
...
> **提示**：选项 `[2]`-`[4]`（下载播放列表：整列 / 连续片段 / 指定集数）都会在保存目录下自动新建一个以**播放列表标题**命名的文件夹，本次列表的所有下载文件都会放入该文件夹。该行为通过命令行临时参数实现：只影响当前这条播放列表 URL，不影响选项 `[1]` 等其他 URL 的下载，也不会写入任何配置文件。
...
| 参数                                 | 原因             |
| ---------------------------------- | -------------- |
| `--output`                         | 由菜单选项 `[7]` 管理；选项 `[2]`-`[4]` 播放列表下载时以临时参数覆盖为「保存目录/播放列表标题/」子目录，不落盘 |
...
- ✨ **播放列表下载自动分目录**：选项 `[2]`-`[4]`（下载整列 / 连续片段 / 指定集数）时，自动在保存目录下新建以**播放列表标题**命名的文件夹，本次列表的所有下载文件自动归入该文件夹
- 该行为通过命令行临时参数实现：不影响选项 `[1]` 单个视频等其他 URL 的下载，也不会写入任何配置文件
- 重复下载同一播放列表时自动复用已有文件夹，不会重复建目录
